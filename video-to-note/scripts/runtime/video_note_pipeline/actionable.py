...
def validate_actionable_payload(
    ...
    if len(chapters) > 12:
        raise ActionableValidationError("自然切章最多允许 12 章")
    ...
    for index, chapter in enumerate(chapters):
        ...
        validate_actionable_chapter(chapter, index, require_frames)
...
def validate_actionable_chapter(
    ...
    if template_type is not None:
        ...
        if template_type == "concept" and "feynman_scaffolding" in chapter:
    ...
    if unit_type in {"operation", "concept"}:
    ...
    elif unit_type == "concept" and "evidence" in chapter:
